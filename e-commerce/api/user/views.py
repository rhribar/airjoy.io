...
from django.views.decorators import csrf_exempt #to make a change here in a file # new
...
def signin(request):
    ...
    #validation part
    if not re.match("/\b[\w\.-]+@[\w\.-]+\.\w{2,4}\b/gi", username): #matches the pattern not the word by word
    ...
    try:
        ...
        if user.check_password(password):
            ...
            if user.session_token != "0": #what is this!??!??! reseting a token value for a user
            ...
            user.save()
            login(request, user)
            return JsonResponse({'token': token, 'user': user_dict}) #you need to throw a token back
        else:
